...
from backup import get_backup_root, get_backup_config, backup_status
...
def run_checks(rounded_values, env, output, pool, domains_to_check=None):
	...
	shell('check_call', ["/usr/sbin/unbound-control", "-q", "flush_zone", "."], trap=True)
...
def run_network_checks(env, output):
	...
	zen = query_dns(rev_ip4+get_spamhaus_query_url(env, 'zen'), 'A', nxdomain=None, retry = False)
...
def check_miab_version(env, output):
	...
	if config.get("privacy", True):
		output.print_warning(f"You are running version Mail-in-a-Box {this_ver} Kiekerjan Edition. Mail-in-a-Box version check disabled by privacy setting.")
	else:
		...
		if this_ver_tag == latest_ver:
			output.print_ok(f"Mail-in-a-Box is up to date. You are running version {this_ver} Kiekerjan Edition.")
		elif latest_ver is None:
			output.print_error(f"Latest Mail-in-a-Box version could not be determined. You are running version {this_ver} Kiekerjan Edition.")
		else:
