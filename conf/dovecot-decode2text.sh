...
# Attachment decoder for Dovecot FTS. Attachment on stdin, UTF-8 on stdout.
# Dovecot 2.4 no longer ships this as an example, so we carry the 2.3 one.
# Installed by setup/dovecot-fts-flatcurve.sh as /usr/lib/dovecot/decode2text.sh
# (next to xml2text, which dovecot-core still provides).
